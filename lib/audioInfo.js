...
function AudioInfo(config) {
  ...
  this.id = `audio_sprite_${loaderUtils.getHashDigest.apply(null, [this.resourcePath].concat(hashOptions))}`;
        ...
        .then(function(ffResponse) {
          if ( _this.metadataFilePath) {
            fs.access(_this.metadataFilePath, function(err) {
              if (!err) {
                _this.metadataFileId = loaderUtils.getHashDigest.apply(null, [_this.metadataFilePath].concat(hashOptions));
              }
  ...
  _initSync: function() {
    ...
    if (this.metadataFilePath) {
      try {
        ...
        this.metadataFileId = loaderUtils.getHashDigest.apply(null, [this.metadataFilePath].concat(hashOptions));
      } catch (err) {
